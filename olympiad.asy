...
void pathticks(path g, int n=1, real r=.5, real spacing=6, real s=8, pen p=currentpen)
{ 
	//picture pict; 
	pair A,B,C,direct; 
...
void czechrightangle(pair A, pair B, pair C, real scale = 0.125){
	pair M = B + unit(A-B)*scale;
	pair N = B + unit(C-B)*scale;
	
	draw(arc(B, M, N));
	draw((4*((M+N)/2)+B) / 5, linewidth(1));
	
}

void parallelticks(path g, real r = 0.5, real spacing = 0.5, real s = 1.5, pen p=currentpen){
	pathticks(g, 2, r, spacing, s, p);
}